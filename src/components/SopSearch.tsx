      ...
      <div className="max-w-4xl mx-auto w-full space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
          ...
          <div className="flex items-center justify-between mt-3 text-[10px] text-slate-400 font-mono">
            <span>READY TO SCAN {sops.length} COOPERATIVE PROCEDURES</span>
            {query && (
              <button 
                onClick={() => setQuery('')}
        ...
        <div className="space-y-4 pb-20">
          <div className="text-[10px] font-mono font-black text-slate-400 uppercase tracking-wider text-left">
            {query.trim() ? `Search Results (${searchResults.length} Matches)` : `All Available Guides (${sops.length} loaded)`}
          </div>
          ...
          {searchResults.length > 0 ? (
            searchResults.map((sop) => {
              ...
              return (
                <motion.div
                  ...
                  className="group relative bg-white rounded-2xl border border-slate-200 p-5 text-left cursor-pointer transition-all shadow-sm hover:shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                >
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center space-x-2.5 flex-wrap gap-y-1">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase ${badgeStyles()}`}>
                        {sop.category}
                      </span>
                    </div>
                    ...
                    <div className="text-[10px] font-mono text-slate-400 flex items-center space-x-2">
                      ...
                      <span>•</span>
                      <span>{questionsCount} Decision Nodes</span>
                    </div>
                  </div>
                  ...
                  <div className="shrink-0 flex justify-end">
                    <button
                      ...
                      className="flex items-center space-x-1.5 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-extrabold text-xs shadow-sm transition-all focus:outline-none"
                    >
            ...
            <div className="bg-white border border-slate-200 rounded-2xl py-12 text-center text-slate-600 shadow-sm">
              <p className="font-bold text-sm">لم يتم العثور على أي نتائج تطابق هذا البحث</p>
              <p className="text-xs text-slate-400 mt-1">جرب كلمات أخرى مثل Radio, Signal, Brake, Fire أو كود الدليل.</p>
            </div>
          )}
        </div>
